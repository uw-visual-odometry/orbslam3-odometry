...
class MinimalPublisher(Node):
    ...
    def __init__(self):
        ...
        # Load camera info
        camera_info_path = Path(self.get_parameter("camera_info_file").get_parameter_value().string_value)
        
        if not camera_info_path.exists():
            self.get_logger().error(f"Camera info file \"{camera_info_path}\" doesn't exist!")
            exit(-1)
