...
double StereoSlamNode::computeEntropy(const cv::Mat& img_gray) {
    cv::Mat hist;
    int histSize = 256;
    float range[] = { 0, 256 };
...
cv::Mat StereoSlamNode::sharpen_block(const cv::Mat& input_image) {
    cv::Mat kernel = (cv::Mat_<float>(3, 3) << 
                       0, -1,  0,
                      -1,  5, -1,
                       0, -1,  0);
    cv::Mat result;
    cv::filter2D(input_image, result, -1, kernel);
    return result;
}

// Main processing function
cv::Mat StereoSlamNode::process_image_entropy_sharpen(const cv::Mat& image, int block_size = 64, double entropy_threshold = -1, double delta = 0.9) {
    cv::Mat gray;
    if (image.channels() == 3)
    ...
    // First pass: compute entropies
    for (int y = 0; y < h; y += block_size) {
        for (int x = 0; x < w; x += block_size) {
            int y_end = std::min(y + block_size, h);
            int x_end = std::min(x + block_size, w);
            cv::Mat block = gray(cv::Range(y, y_end), cv::Range(x, x_end));
            entropies.push_back(computeEntropy(block));
        }
    }

    // Adaptive threshold
    if (entropy_threshold < 0) {
        double mean_entropy = std::accumulate(entropies.begin(), entropies.end(), 0.0) / entropies.size();
        entropy_threshold = mean_entropy + delta;
    }

    // Second pass: sharpen if entropy < threshold
    for (int y = 0; y < h; y += block_size) {
        for (int x = 0; x < w; x += block_size) {
            int y_end = std::min(y + block_size, h);
            int x_end = std::min(x + block_size, w);
            cv::Mat block = processed(cv::Range(y, y_end), cv::Range(x, x_end));
            if (computeEntropy(block) < entropy_threshold) {
                cv::Mat sharpened = sharpen_block(block);
                sharpened.copyTo(processed(cv::Range(y, y_end), cv::Range(x, x_end)));
            }
        }
    }
...
void StereoSlamNode::syncedCallback(
  ...
  //Information entropy with adaptive sharpening
  // cv::Mat left_gray;
  // cv::Mat right_gray;
  // cv::cvtColor(left_image, left_gray, cv::COLOR_BGR2GRAY);
  // cv::cvtColor(right_image, right_gray, cv::COLOR_BGR2GRAY);
  // double entropy_left = computeEntropy(left_gray);
  // double entropy_right = computeEntropy(right_gray);
  // left_image = adaptiveSharpening(left_image, entropy_left);
  // right_image = adaptiveSharpening(right_image, entropy_right);
  left_image = process_image_entropy_sharpen(left_image, 64, -1, 0.6);
  right_image = process_image_entropy_sharpen(right_image, 64, -1, 0.6);
  ...
  std::cout << "Elapsed time: " << tempo << " milliseconds" << std::endl;
...
sensor_msgs::msg::Range StereoSlamNode::pointcloud_to_depth(
  ...
  std::cout << "valid_points: " << valid_points << std::endl;
  if (valid_points > 0) {
  ...
  bool lost_bool = m_SLAM->isLost();
  if (lost_bool || valid_points<15) { 
    lost_count++;
  }
  std::cout << "Lost count: " << lost_count << std::endl;
...
// /**
//  * Information entropy and adaptive sharpening to help increase feature detection
//  */
// // double StereoSlamNode::computeEntropy(const cv::Mat& img_gray) {
...
// //     int total_pixels = img_gray.rows * img_gray.cols;
// //     double entropy = 0.0;

// //     for (int i = 0; i < 256; ++i) {
// //         if (hist[i] > 0) {
// //             double p = (double)hist[i] / total_pixels;
// //             entropy -= p * std::log2(p);
// //         }
// //     }
...
// // cv::Mat adaptiveSharpening(const cv::Mat& input_image, double entropy, double entropy_threshold = 6.0) {
// //     if (entropy >= entropy_threshold)
// //         return input_image;  // No sharpening needed
...
// //     cv::filter2D(input_image, sharpened, input_image.depth(), kernel);
// //     return sharpened;
// // }

// // Apply sharpening kernel to an image block
// cv::Mat StereoSlamNode::sharpen_block(const cv::Mat& input_image) {
//     cv::Mat kernel = (cv::Mat_<float>(3, 3) << 
//                        0, -1,  0,
//                       -1,  5, -1,
