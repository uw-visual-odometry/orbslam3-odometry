...
The only input is path_yaml, that must be the path of the settings file of
ORB-SLAM3. All the other variables (map1_L,map2_L,roi_L, map1_R, map2_R, roi_R)
are the output of the stereoRectification and undistortion. The first three
outputs (map1_L,map2_L,roi_L) are for the left camera (aka Camera1), while the
other three are for the right (aka Camera2).
*/
void readParameters(std::string path_yaml, cv::Mat &map1_L, cv::Mat &map2_L,
                    cv::Rect &roi_L, cv::Mat &map1_R, cv::Mat &map2_R,
                    cv::Rect &roi_R) {
  ...
  cv::Mat camera_matrix_1 =
      (cv::Mat_<double>(3, 3, CV_64F) << fx, 0, cx, 0, fy, cy, 0, 0, 1);
  ...
  // Check tipo
  std::cout << "camera_matrix_1.type():\t" << camera_matrix_1.type()
            << std::endl;
  std::cout << "dist_coeffs_1.type():\t" << dist_coeffs_1.type() << std::endl;

  std::cout << "camera_matrix_2.type():\t" << camera_matrix_2.type()
            << std::endl;
  ...
  try {
    cv::stereoRectify(camera_matrix_1, dist_coeffs_1, camera_matrix_2,
                      dist_coeffs_2, cv::Size(camera_width, camera_height), R,
                      T, R1, R2, P1, P2, Q, cv::CALIB_ZERO_DISPARITY, -1,
                      cv::Size(), &roi_L, &roi_R);
  ...
  cv::initUndistortRectifyMap(camera_matrix_1, dist_coeffs_1, R1, P1,
                              cv::Size(camera_width, camera_height), CV_32FC1,
                              map1_L, map2_L);
  cv::initUndistortRectifyMap(camera_matrix_2, dist_coeffs_2, R2, P2,
                              cv::Size(camera_width, camera_height), CV_32FC1,
                              map1_R, map2_R);

  std::cout << "All parameter have been read. You can now perform remapping"
            << std::endl;
}

/**
This function performs rectification of the image.
- cv::Mat &img is both an input and output value. The expected image is the
image that you want to rectify and crop. The output image is the rectified and
cropped image.
- img_non_cropped is an output value. It contains the rectified (and NOT
cropped) image.
- map1, map2, roi are the output of the readParameters function.

Tip: in order to have left and right cropped images of the same size, in the
parameter cv::Rect &roi provide (roi_L & roi_R) for both cameras.
*/
void rectify_image(cv::Mat &img, const cv::Mat &map1, const cv::Mat &map2,
                   const cv::Rect &roi, cv::Mat &img_non_cropped) {
  // In img -> saves rectified and cropped image in img.
  // In img_non_cropped -> saves rectified and NON cropped image in
  // img_non_cropped (for disparity map)
...
img is still both an input and output value. The expected image is still the
image that you want to rectify and crop. The output image is the rectified and
cropped image.
*/
void rectify_image(cv::Mat &img, const cv::Mat &map1, const cv::Mat &map2,
...
THE IMAGES MUST BE OF THE SAME SIZE. To have images of the same size you can
provide:
- rectified but not cropped images
or
- rectified and cropped images, where the crop was made with the roi=(roi_L &
roi_R)
*/
void show_disparity(const cv::Mat &img1_non_cropped,
                    const cv::Mat &img2_non_cropped) {
  cv::Mat disparityMap;
  const int numDisparitiesVecchio = 16;
  const int blockSizeVecchio = 15;
  cv::Ptr<cv::StereoSGBM> sgbmVecchio =
      cv::StereoSGBM::create(0, numDisparitiesVecchio, blockSizeVecchio);
  ...
  /*std::cout << "Image 1 dimensions: " << img1_non_cropped.size() << ", type: "
  << img1_non_cropped.type() << std::endl; std::cout << "Image 2 dimensions: "
  << img2_non_cropped.size() << ", type: " << img2_non_cropped.type() <<
  std::endl; std::cout << "Image 3 dimensions: " << disparityMap.size() << ",
  type: " << disparityMap.type() << std::endl;*/
  ...
  const int numDisparities =
      16 * 5;               // Maximum disparity minus minimum disparity
  ...
  cv::resize(concatenated, concatenated,
             cv::Size(concatenated.cols / 2, concatenated.rows / 2));
