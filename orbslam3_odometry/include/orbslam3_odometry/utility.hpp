...
class Utility {
 public:
  ...
  static void cropping_image(cv::Mat &image, cv::Rect roiRect) {
    image = image(roiRect);
  }
...
#define TOSTRING(x) STRINGIFY(x)
    ...
    std::cout << "****\n\nYou are using images of type: " << TOSTRING(ImageMsg)
              << ".\nIf you are using compresssed images, remember that you "
                 "will not visualize them in rviz2"
              << std::endl;

    if (qos.get_rmw_qos_profile().reliability ==
        RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT) {
      std::cout << "QoS is set to best effort." << std::endl;
    } else if (qos.get_rmw_qos_profile().reliability ==
